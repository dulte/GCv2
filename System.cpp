...
System::System(InputReader* &input)
{
  ...
  outFileStripes.open("output/statestripes.bin", ios::out | ios::binary);
...
void System::run()
{
  int numberOfStripes =  inputReader->numberOfStripes;//(floor((inputReader->blockWidth)/2) + 1);
  cout << (numberOfStripes == 11) << endl;
  double stripes[numberOfStripes];//int(floor(double(inputReader->blockWidth)/2) + 1)];
  cout << sizeof(stripes) << endl;
  ...
  while (t<inputReader->tStop)
  {
    ...
    for (shared_ptr<Block> block: blocks)
    {
      ...
      //cout << "he" << endl;
      if (block->yID == 0 && block->xID%2 == 0)
      {
        stripes[int(floor(double(block->xID)/2))] = block->getPercentageSlipped();
      }
    ...
    if ((counter%inputReader->writeFrequency) == 0)
    {
      ...
      writeArrayToFile(outFileStripes,stripes,numberOfStripes);//floor(double(inputReader->blockWidth/2)) + 1);
